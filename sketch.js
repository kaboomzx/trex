...
function setup() {
  ...
  ground = createSprite(200,height/2 - 10 ,400,20);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((keyDown("space")|| touches.length > 0) && trex.y >= height / 2 -50) {
      trex.velocityY = -12;
      touches = []
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(600,height/2 - 100,40,10);
    cloud.y = Math.round(random(100,height/2 -100));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(600,height/2 - 25,10,40);
    //obstacle.debug = true;
